...
```
/workspace/
├── Buono.py                          # Script principale per generazione dataset
...
Lo script `Buono.py` genera automaticamente il dataset combinando:
1. **Tile satellitari** da ESRI World Imagery
2. **Geometrie strade** da file OSM (`.pbf`)

### Configurazione

Modifica le seguenti variabili in `Buono.py`:
...
Lo script `Buono.py` usa un algoritmo di campionamento casuale con controllo:
